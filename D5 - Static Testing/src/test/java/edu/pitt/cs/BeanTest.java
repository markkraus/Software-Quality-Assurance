...
public class BeanTest {
  ...
  static final int beanCount = 4;
  static final int slotCount = 10;
  ...
  public void setUp() {
    ...
    // Call Bean.createInstance to create a bean in skilled mode for slotCount and
    // assign to beans[1].
    // To fix bean skill level to 0, pass in a rand which always returns -3.0 on
    // rand.nextGaussian().
    ...
    // Call Bean.createInstance to create a bean in skilled mode for slotCount and
    // assign to beans[2].
    // To fix bean skill level to 2, pass in a rand which always returns -1.5 on
    // rand.nextGaussian().
    ...
    // Call Bean.createInstance to create a bean in skilled mode for slotCount and
    // assign to beans[3].
    // To fix bean skill level to 9, pass in a rand which always returns 3.0 on
    // rand.nextGaussian().
  ...
  @Test
  public void testAdvanceStepLuckyBean10TimesLeft() {
    Mockito.when(rand.nextInt(2)).thenReturn(0);
    boolean exceptionThrown = false;
    boolean unwantedExceptionThrown = false;
    ...
    for (int i = 0; i < 9; i++) {
      try {
        ...
        unwantedExceptionThrown = true;
      }
    }

    // On the 10th call, check that a BeanOutOfBoundsException is thrown
    try {
      beans[0].advanceStep();
    } catch (BeanOutOfBoundsException e) {
    ...
    // Assert that the exception was thrown on the 10th call
    assertTrue(exceptionThrown);
    assertTrue(!unwantedExceptionThrown);
  }
