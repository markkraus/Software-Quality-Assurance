...
public class JPFJUnitTest extends TestJPF {
  ...
  private int slotCount; // The number of slots in the machine we want to test
  private int beanCount; // The number of beans in the machine we want to test
  private boolean isLuck; // Whether the machine we want to test is in "luck" or "skill" mode
  ...
  public void testReset() {
    ...
    // This is the JPF VM, so run the test case on top of it, starting from the
    // setUp().
    setUp();
   ...
   * 
   * <pre>
   * Preconditions: logic has been initialized with an instance of
   * BeanCounterLogic.
   * beans has been initialized with an array of Bean objects.
   * Execution steps: Call logic.reset(beans).
   ...
   * (For example, getInFlightBeanXPos(0) can either return 0 or
   * BeanCounterLogic.NO_BEAN_IN_YPOS,
   * and getInFlightBeanXPos(1) can return 0, 1, or
   * BeanCounterLogic.NO_BEAN_IN_YPOS. And so on.)
   * </pre>
   */
    ...
    while (logic.advanceStep()) {
      for (int i = 0; i < slotCount; i++) { // outer for loop checks each row
        ...
        for (int j = 0; j <= i; j++) { // checks if bean on that row has a valid x coord
          if (logic.getInFlightBeanXPos(i) == j) {
            validRow = true;
          }
        }
        assertTrue(validRow);
  ...
  public void testLowerHalf() throws BeanOutOfBoundsException {
    ...
    if (beanCount % 2 == 0) {
      amountToCut = beanCount / 2;
    } else {
      amountToCut = (beanCount - 1) / 2;
    }
    ...
    for (int i = slotCount - 1; i >= 0; i--) {
      ...
      if (current > amountToCut) {
        expectedSlotCounts[i] = current - amountToCut;
        amountToCut = 0;
      } else {
        amountToCut = amountToCut - current;
   ...
   *                beans has been initialized with an array of Bean objects.
   * Execution steps: Call logic.reset(beans).
   *                  Store logic.getRemainingBeanCount() in int preSize
   *                  Call logic.advanceStep() in a loop until it returns false (the machine terminates).
   *                  Add a bean to the queue 
   *                  Call logic.repeat();
   * Invariants: (preSize + 1) equals logic.getRemainingBeanCount().
   * </pre>
   */

  @Test
  public void testRepeatCont() throws BeanOutOfBoundsException {
    setUp();
    if (verifyNoPropertyViolation() == false) {
      return;
    }
    logic.reset(beans);
    int preSize = logic.getRemainingBeanCount();
    // Intentionally leave one bean in the queue
    while (logic.getRemainingBeanCount() > 1) {
      logic.advanceStep();
    }
    logic.repeat();

    // Repeat should add all dropped beans in the queue without erasing the one left
    // from before
    assertEquals(preSize, logic.getRemainingBeanCount());
  }
}